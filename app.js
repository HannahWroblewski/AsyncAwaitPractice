...
async function processRepOne( targetElement ) {
  ...
  $('#rep-three input').on('input',  async function () {
    try {
        const newFunc = await asyncReverse ($(this).val());
        const newText = await asyncExcite(newFunc);
        $('#rep-three .message').text(newText);
    } catch (error) {
        console.error(error);
    }
  });
  ...
  $('#rep-four input').on('input', async function () {
    ...
    try{
      ...
      $('#rep-four .info').html(
        $(
          `<h2>${user.username}</h2>${posts.map((post) => `<p>${post.text}</p>`).join("")}`
        )
      );
    } catch (error) {
      console.log(error);
